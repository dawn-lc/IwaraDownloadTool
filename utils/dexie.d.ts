...
type IndexableTypeArray = Array<IndexableTypePart>;
 ...
 *
 */
interface PromiseExtended<T = any> extends Promise<T> {
...
type DBCoreMutateRequest = DBCoreAddRequest | DBCorePutRequest | DBCoreDeleteRequest | DBCoreDeleteRangeRequest;
interface DBCoreMutateResponse {
	numFailures: number;
	failures: {
...
interface DBCoreOpenCursorRequest {
	trans: DBCoreTransaction;
	values?: boolean;
	unique?: boolean;
	reverse?: boolean;
...
interface DBCoreSchema {
	name: string;
	tables: DBCoreTableSchema[];
}
interface DBCoreTableSchema {
	readonly name: string;
...
interface Dexie extends Database {
	readonly name: string;
	readonly tables: Table[];
...
declare module Dexie {
	...
	// The "Dexie.Table" interface. Same as named ed interface Table.
	interface Table<T = any, Key = any> extends _Table<T, Key> {
	} // Because all samples have been Dexie.Table<...>
	// The "Dexie.Collection" interface. Same as named ed interface Collection.
